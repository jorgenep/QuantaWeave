"""Hyperparameter sweeps over MoE settings, run through the experiment manager.
  ...
  name: moe-sweep
  strategy: grid            # grid | random | halving | bayes
...
listed (bayes never proposes them). `bayes` fits a Gaussian process to the trials so far (bayes_opt.py) and runs the
point with the highest expected improvement next, so it is sequential; n_trials is the total budget.
"""
...
from experiment_manager import DEFAULT_RUNS_DIR, compare_runs, dig, load_config, load_summaries, run_experiment
...
def run_sweep(config: dict, runs_dir: Path = DEFAULT_RUNS_DIR, dry_run: bool = False) -> dict:
    ...
    if strategy not in {"grid", "random", "halving", "bayes"}:
        raise ValueError("strategy must be grid, random, halving or bayes")
