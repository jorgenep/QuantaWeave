"""Benchmark a saved AstraForge sparse-MoE checkpoint."""
...
from train_astraforge_moe import CharacterDataset, select_device
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark an AstraForge MoE checkpoint")
    parser.add_argument("--checkpoint", type=Path, default=Path("artifacts/outputs/astraforge-moe-out"))
    ...
    config = AstraForgeConfig(**json.loads(config_path.read_text()))
